...
<?php
   if(isset($_GET['id_package'])){
       $id = $_GET['id_package'];
       $get = "SELECT max, price, id_ctv FROM package WHERE id= $id AND type='SHARE'";
   ...
   if(isset($_POST['submit'])){
      ...
      $price = $_POST['price'];
      $sql = "UPDATE package SET price='$price', max='$max_share' WHERE id=$id";
      if(mysqli_query($conn, $sql)){
          $content = "<b>$uname</b> vừa cập nhật Package <b>VIP Share</b>, Max Share: <b> $max_share </b> Likes, Giá tiền: <b> ".number_format($price)." VNĐ </b>";
          $time = time();
          $his = "INSERT INTO history(content, id_ctv, time) VALUES('$content', '$idctv','$time')";
          if(mysqli_query($conn, $his)){
              echo "<script>alert('Thành công'); window.location='index.php?DS=List_Package_Share';</script>";
          }
      }
   }
?>
    ...
    <div class="col-md-12">
        ...
        <div class="box box-info wow fadeIn">
            <div class="box-header with-border">
                <h3 class="box-title">Cập nhật Package VIP Share</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form class="form-horizontal" action="#" method="post">
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                           <input type="number" name="max_share" max="10000" class="form-control" value="<?php echo isset($x['max']) ? $x['max'] : ''; ?>" required="" />
                        </div>
                    </div>
