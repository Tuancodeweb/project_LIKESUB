...
<script>
    function check(id) {
        if (confirm('Bạn có chắc chắn muốn xóa package này ?') == true) {
            window.location = 'index.php?DS=Delete_Package_CMT&id_package=' + id;
        } else {
            return false;
        }
    }
...
<div class="box wow fadeIn">
    <div class="box-header">
        <h3 class="box-title">Danh sách Package CMT | <a href="index.php?DS=Add_Package_CMT" class="btn btn-danger">Thêm Package</a></h3>
    </div>
    <!-- /.box-header -->
                ...
                <tbody>
                    ...
                    while ($x = mysqli_fetch_assoc($result)) {
                        ...
                        <tr style="font-weight: bold">
                            <td><?php echo $x['id']; ?></td>
                            <td><?php echo $x['max']; ?> CMTs</td>
                            <td><?php echo number_format($x['price']); ?> VNĐ</td>
                            <td>VIP CMT</td>
                            <?php echo isset($u_name, $name) ?  "<td>$name ($u_name)</td>" : ''; ?>
                            <td style="text-align:center"><a href="index.php?DS=Update_Package_CMT&id_package=<?php echo $id; ?>" class="btn btn-info">Sửa</a> <a href="#" onclick="check(<?php echo $id; ?>);" class="btn btn-danger">Xóa</a></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
